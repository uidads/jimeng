...
export class ServiceConfig {
    ...
    get addressHost() {
        ...
        // 根据环境返回不同的默认IP
        const env = require('../environment.ts').default.env;
        if (env === 'prod') {
            return ipAddresses[0] || "149.88.88.161";
        } else {
    ...
    get pageDirUrl() {
        return `http://${this.addressHost}:${this.port}/page`;
    }

    get publicDirUrl() {
        return `http://${this.addressHost}:${this.port}/public`;
    }
